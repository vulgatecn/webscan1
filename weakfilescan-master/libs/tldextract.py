...
CACHE_FILE = os.path.expanduser(
    os.environ.get("TLDEXTRACT_CACHE", CACHE_FILE_DEFAULT))
...
class ExtractResult(collections.namedtuple('ExtractResult', 'subdomain domain suffix')):
    ...
    def tld(self):
        warnings.warn(
            'This use of tld is misleading. Use `suffix` instead.', DeprecationWarning)
        return self.suffix
...
class TLDExtract(object):
    ...
    def __init__(self, cache_file=CACHE_FILE, suffix_list_url=PUBLIC_SUFFIX_LIST_URLS, fetch=True,
        ...
        self.suffix_list_urls = tuple(
            url.strip() for url in self.suffix_list_urls if url.strip())
...
def get_tlds_from_raw_suffix_list_data(suffix_list_source, include_psl_private_domains=False):
    if include_psl_private_domains:
        ...
        text, _, _ = suffix_list_source.partition(
            '// ===BEGIN PRIVATE DOMAINS===')

    tld_finder = re.compile(
        r'^(?P<tld>[.*!]*\w[\S]*)', re.UNICODE | re.MULTILINE)
...
def fetch_file(urls):
    ...
    for url in urls:
        try:
            ...
            LOG.error(
                'Exception reading Public Suffix List url ' + url + ' - ' + str(e) + '.')
        else:
            return _decode_utf8(s)

    LOG.error(
        'No Public Suffix List found. Consider using a mirror or constructing your TLDExtract with `fetch=False`.')
    return unicode('')
...
def main():
    ...
    parser.add_argument(
        '--version', action='version', version='%(prog)s ' + distribution.version)
    parser.add_argument('input', metavar='fqdn|url',
                        type=unicode, nargs='*', help='fqdn or url')

    parser.add_argument('-u', '--update', default=False,
                        action='store_true', help='force fetch the latest TLD definitions')
    parser.add_argument(
        '-c', '--cache_file', help='use an alternate TLD definition file')
